...
module.exports = new (class viewController extends InitializeController {
  async view(req, res) {
    //
    try {
      let query = {};
      ...
      for (let index = start; index <= moment; index += 86400000) {
        const day = await this.model.View.countDocuments({
          ...query,
        ...
        if (index >= Date.parse(moment)) {
          ...
          const results = await this.model.View.countDocuments(query).exec();
