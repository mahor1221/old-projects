...
module.exports = new (class downloadController extends InitializeController {
  async download(req, res) {
    ...
    try {
      ...
      for (let index = start; index <= moment; index += 86400000) {
        const day = await this.model.Download.countDocuments({
          ...query,
        ...
        if (index >= Date.parse(moment)) {
          ...
          const results = await this.model.Download.countDocuments(query).exec();
